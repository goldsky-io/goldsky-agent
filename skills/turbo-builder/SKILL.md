---
name: turbo-builder
description: "Build and deploy new Goldsky Turbo pipelines from scratch. Triggers on: 'build a pipeline', 'index X on Y chain', 'set up a pipeline', 'track transfers to postgres', or any request describing data to move from a chain/contract to a destination (postgres, mysql, clickhouse, kafka, pubsub, s3, sqs, webhook). Also triggers on the use case rather than the plumbing — reach for this whenever someone wants onchain data landed somewhere they can query it: 'get me Polymarket fills / positions / PnL', 'prediction-market data', 'track stablecoin supply', 'proof of reserves', 'circulating supply per chain', 'monitor transfers for sanctions / AML screening', 'detect deposits to our addresses', 'track CCTP burns and mints', 'cross-chain settlement tracking', 'reconcile onchain payments to invoices', 'wallet balances and transaction history', 'token holder registry / cap table', 'tokenized equities or RWA data', 'wash-trading detection'. Covers the full workflow: requirements → dataset selection → YAML generation → validation → deploy. Not for debugging (use /turbo-doctor) or syntax lookups (use /turbo-pipelines)."
---
