...
| Migrate a subgraph from The Graph                     | `/subgraph-migrate-thegraph` |
...
│   ├── subgraphs/             # Subgraph deploy, GraphQL endpoints, tags, webhooks
│   ├── subgraph-doctor/       # Diagnose and fix failing/stalled subgraphs
│   ├── subgraph-migrate-thegraph/  # Guided migration from The Graph
...
| ----- | ----------- | ------------- |
| `subgraphs` | "Deploy a subgraph / manage GraphQL endpoints / tags / webhooks" | Deploy paths, GraphQL endpoints, tags, webhooks, lifecycle, cross-chain patterns |
| `subgraph-doctor` | "My subgraph stopped syncing / won't deploy / is throwing errors" | Diagnostic workflow: status + log checks, the `_meta` query, error-pattern matching, fixes |
| `subgraph-migrate-thegraph` | "Move my subgraph off The Graph onto Goldsky" | Path selection, deploy, verify sync, migrate tags, swap endpoint |
...
- **Subgraphs** — Deploy, tags, webhooks, endpoints, cross-chain; interactive diagnosis (`subgraph-doctor`) and guided migration from The Graph (`subgraph-migrate-thegraph`)
